...
function c11000223.target(e,tp,eg,ep,ev,re,r,rp,chk)
	...
	if chk==0 then
		...
		local res=Duel.GetLocationCount(tp,LOCATION_MZONE)>0
			and Duel.IsExistingMatchingCard(c11000223.filter2,tp,LOCATION_EXTRA,0,1,nil,e,tp,mg1,nil,c)
		if not res then
			...
			if ce~=nil then
				...
				res=Duel.IsExistingMatchingCard(c11000223.filter2,tp,LOCATION_EXTRA,0,1,nil,e,tp,mg2,mf,c)
			end
		end
		return res
	end
...
function c11000223.operation(e,tp,eg,ep,ev,re,r,rp)
	...
	local mg1=Duel.GetMatchingGroup(c11000223.filter1,tp,LOCATION_HAND,0,c,e)
	local sg1=Duel.GetMatchingGroup(c11000223.filter2,tp,LOCATION_EXTRA,0,nil,e,tp,mg1,nil,c)
	local mg2=nil
	local sg2=nil
	local ce=Duel.GetChainMaterial(tp)
	if ce~=nil then
		...
		mg2=fgroup(ce,e,tp)
		local mf=ce:GetValue()
		sg2=Duel.GetMatchingGroup(c11000223.filter2,tp,LOCATION_EXTRA,0,nil,e,tp,mg2,mf,c)
	end
