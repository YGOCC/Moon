...
function c500312625.initial_effect(c)
	...
	e2:SetTarget(aux.TargetBoolFunction(Card.IsSetCard,0x185a))
...
function c500312625.filter(c,e,tp,m)
	if not c:IsSetCard(0x85a) or bit.band(c:GetType(),0x81)~=0x81 or c:IsType(TYPE_SPELL+TYPE_TRAP)
...
function c500312625.thfilter(c)
	return c:IsSetCard(0x85a)and c:IsAbleToHand() and not c:IsCode(500312625)
end
