...
function c16000795.tdfilter(c)
	return c:IsFaceup() and c:IsType(TYPE_MONSTER) and (c:GetLevel()>0  or c:GetRank()>0 ) and c:IsDestructable()
end
function c16000795.dagtg(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	if chkc then return chkc:IsLocation(LOCATION_MZONE) and chkc:IsControler(1-tp) end
	if chk==0 then return Duel.IsExistingTarget(c16000795.tdfilter,tp,0,LOCATION_MZONE,1,nil) end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_DESTROY)
	local g=Duel.SelectTarget(tp,c16000795.tdfilter,tp,0,LOCATION_MZONE,1,1,nil)
...
function c16000795.damtg(e,tp,eg,ep,ev,re,r,rp,chk)
	...
	if chk==0 then return Duel.IsExistingTarget(c16000795.tdfilter,tp,0,LOCATION_MZONE,1,nil) end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_DESTROY)
	local tc=Duel.SelectTarget(tp,c16000795.tdfilter,tp,0,LOCATION_MZONE,1,1,nil):GetFirst()
	if tc:GetRank()>0 then
