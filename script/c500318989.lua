...
function c500318989.splimit(e,c,sump,sumtype,sumpos,targetp)
	if c:IsSetCard(0x285a)  then return false end
...
function c500318989.filter(c,tp)
	return c:IsSetCard(0x285a) and c:IsControler(tp) and c:GetSummonType()==SUMMON_TYPE_XYZ
end
...
function c500318989.filter1(c,e,tp)
	local rk=c:GetRank()
	return rk>0 and c:IsFaceup()and c:IsSetCard(0x285a) and c:IsType(TYPE_XYZ) and Duel.IsExistingMatchingCard(c500318989.filter2,tp,LOCATION_EXTRA,0,1,nil,rk+1,e,tp,c:GetCode())
...
function c500318989.filter3(c,rk)
	return c:IsFaceup() and c:IsSetCard(0x285a) and c:IsType(TYPE_XYZ) and c:GetRank()>rk
end
