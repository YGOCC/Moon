...
function c50031159.sumop(e,tp,eg,ep,ev,re,r,rp)
	...
	e1:SetTarget(aux.TargetBoolFunction(Card.IsSetCard,0x185a))
...
function c50031159.chainop(e,tp,eg,ep,ev,re,r,rp)
	...
	if re:IsHasType(EFFECT_TYPE_ACTIVATE) and rc:IsSetCard(0x185a) and re:IsActiveType(TYPE_FIELD) then
...
function c50031159.sucfilter(c)
	return c:IsSetCard(0x185a) and c:IsType(TYPE_RITUAL) and bit.band(c:GetSummonType(),SUMMON_TYPE_RITUAL)==SUMMON_TYPE_RITUAL
end
...
function c50031159.thfilter(c)
	return c:IsSetCard(0x185a) and not c:IsCode(50031159)and c:IsType(TYPE_MONSTER) and c:IsAbleToHand()
end
