...
function c90000019.condition(e,tp,eg,ep,ev,re,r,rp)
	return rp~=tp and re:IsHasType(EFFECT_TYPE_ACTIVATE) and Duel.IsChainNegatable(ev)
		and Duel.IsExistingMatchingCard(c90000019.filter,tp,LOCATION_ONFIELD,0,1,nil)
end
...
function c90000019.filter2(c,tp)
...
function c90000019.condition2(e,tp,eg,ep,ev,re,r,rp)
	return rp~=tp and re:GetActivateLocation()==LOCATION_GRAVE and Duel.IsChainNegatable(ev)
		and Duel.IsExistingMatchingCard(c90000019.filter2,tp,LOCATION_GRAVE,0,1,nil)
end
function c90000019.cost(e,tp,eg,ep,ev,re,r,rp,chk)
