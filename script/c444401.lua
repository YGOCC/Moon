...
function c444401.initial_effect(c)
	--copy
	local e1=Effect.CreateEffect(c)
	e1:SetDescription(aux.Stringid(444401,0))
	e1:SetProperty(EFFECT_FLAG_CARD_TARGET)
	e1:SetType(EFFECT_TYPE_IGNITION)
	e1:SetRange(LOCATION_ONFIELD+LOCATION_PZONE)
	e1:SetCountLimit(1)
	e1:SetCondition(c444401.condition)
	e1:SetTarget(c444401.target)
	e1:SetOperation(c444401.operation)
	c:RegisterEffect(e1)
	-- ATK UP
	local e2=Effect.CreateEffect(c)
	e2:SetType(EFFECT_TYPE_FIELD)
	e2:SetRange(LOCATION_ONFIELD+LOCATION_PZONE)
	e2:SetCondition(c444401.passivescondition)
	e2:SetTargetRange(LOCATION_MZONE,0)
	e2:SetCode(EFFECT_UPDATE_ATTACK)
	e2:SetTarget(c444401.tg1)
	e2:SetValue(200)
	c:RegisterEffect(e2)
end
-- "global handlerspecific activation limiter" (archetype effect)
function c444401.condition(e,tp,eg,ep,ev,re,r,rp)
	local c=e:GetHandler()
	local eqc=c:GetEquipTarget()
	return  Duel.GetFlagEffect(tp,e:GetHandler():GetCode())<2 and not eqc 
end
function c444401.passivescondition(e,tp,eg,ep,ev,re,r,rp)
	local c=e:GetHandler()
	local eqc=c:GetEquipTarget()
	return  not eqc 
end
-- copy functions: target
function c444401.target(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
...
function c444401.copyfilter(c)
	return c:IsFaceup() and c:IsSetCard(4444)
end
-- copy functions: operation
function c444401.operation(e,tp,eg,ep,ev,re,r,rp)
    local c=e:GetHandler()
    ...
    if tc and c:IsRelateToEffect(e) and c:IsFaceup() and tc:IsRelateToEffect(e) and tc:IsFaceup() and not tc:IsType(TYPE_TOKEN) then
        ...
        local e2=Effect.CreateEffect(c)
        e2:SetDescription(aux.Stringid(444401,1))
        ...
        e2:SetLabel(cid)
        e2:SetOperation(c444401.rstop)
        c:RegisterEffect(e2)
    end
end
-- copy functions: reset
function c444401.rstop(e,tp,eg,ep,ev,re,r,rp)
    local c=e:GetHandler()
