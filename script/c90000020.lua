...
function c90000020.initial_effect(c)
	...
	e2:SetTarget(c90000020.target)
	e2:SetOperation(c90000020.operation)
	c:RegisterEffect(e2)
end
function c90000020.filter1(c,e)
	return c:IsOnField() and not c:IsImmuneToEffect(e)
end
function c90000020.filter2(c,e,tp,m,f,gc)
...
function c90000020.target(e,tp,eg,ep,ev,re,r,rp,chk)
	...
	if chk==0 then
		...
		local res=Duel.IsExistingMatchingCard(c90000020.filter2,tp,LOCATION_EXTRA,0,1,nil,e,tp,mg1,nil,c)
		if not res then
			...
			if ce~=nil then
				...
				res=Duel.IsExistingMatchingCard(c90000020.filter2,tp,LOCATION_EXTRA,0,1,nil,e,tp,mg2,mf,c)
			end
		end
		return res
	end
...
function c90000020.operation(e,tp,eg,ep,ev,re,r,rp)
	...
	local mg1=Duel.GetFusionMaterial(tp):Filter(c90000020.filter1,c,e)
	local sg1=Duel.GetMatchingGroup(c90000020.filter2,tp,LOCATION_EXTRA,0,nil,e,tp,mg1,nil,c)
	local mg2=nil
	local sg2=nil
	local ce=Duel.GetChainMaterial(tp)
	if ce~=nil then
		...
		mg2=fgroup(ce,e,tp)
		local mf=ce:GetValue()
		sg2=Duel.GetMatchingGroup(c90000020.filter2,tp,LOCATION_EXTRA,0,nil,e,tp,mg2,mf,c)
	end
