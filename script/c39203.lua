...
function scard.initial_effect(c)
	...
	e1:SetDescription(aux.Stringid(25542642,0))
...
function scard.spfilter(c,e,tp)
	return ((c:IsSetCard(0xa3) and c:IsType(TYPE_SYNCHRO))) or (c:IsSetCard(0x43)) and c:IsCanBeSpecialSummoned(e,0,tp,false,false)
end
function scard.sptg(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	...
	if chk==0 then return Duel.GetLocationCount(tp,LOCATION_MZONE)>0
		and Duel.IsExistingTarget(scard.spfilter,tp,LOCATION_GRAVE,0,1,e:GetHandler(),e,tp) end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_SPSUMMON)
	local g=Duel.SelectTarget(tp,scard.spfilter,tp,LOCATION_GRAVE,0,1,1,e:GetHandler(),e,tp)
