...
function cid.initial_effect(c)
	...
	e2:SetCondition(function(e,tp,eg) local tc=eg:GetFirst() return #eg==1 and tc:IsType(TYPE_FUSION) and tc:IsSummonType(SUMMON_TYPE_FUSION) end)
...
function cid.filter(c)
	return c:IsSetCard(0x50b) and not c:IsCode(id) and c:IsAbleToHand()
end
...
function cid.thfilter(c)
	return c:GetType()&0x83>0x80 and c:IsAbleToHand()
end
