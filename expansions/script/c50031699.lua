...
function c50031699.initial_effect(c)
	...
	e4:SetDescription(aux.Stringid(500314225,0))
	...
	e4:SetOperation(c500314225.operation2)
	c:RegisterEffect(e4)
end
...
function c50031699.operation(e,tp,eg,ep,ev,re,r,rp)
	...
	if tc and Duel.SpecialSummonStep(tc,0,tp,tp,false,false,POS_FACEUP) then
		...
		e3:SetCode(EFFECT_CANNOT_BE_SYNCHRO)
...
function c50031699.target2(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	if chkc then return chkc:IsLocation(LOCATION_REMOVED) and chkc:IsControler(tp) and c50031699.filter(chkc) end
	if chk==0 then return Duel.IsPlayerCanDraw(tp,1)
		and Duel.IsExistingTarget(c50031699.filter,tp,LOCATION_REMOVED,0,5,nil) end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_TODECK)
	local g=Duel.SelectTarget(tp,c50031699.filter,tp,LOCATION_REMOVED,0,5,5,nil)
