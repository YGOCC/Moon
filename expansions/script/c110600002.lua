...
function cm.pthop(e,tp,eg,ep,ev,re,r,rp)
	...
	local g=Duel.SelectMatchingCard(tp,cm.prethfilter,tp,LOCATION_DECK,0,1,1,nil)
...
function cm.sctg(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.IsExistingMatchingCard(cm.prethfilter,tp,LOCATION_DECK,0,1,nil) end
end
function cm.scop(e,tp,eg,ep,ev,re,r,rp)
	local c=e:GetHandler()
	if not c:IsRelateToEffect(e) then return end
	Duel.Hint(HINT_SELECTMSG,tp,aux.Stringid(m,1))
	local g=Duel.SelectMatchingCard(tp,cm.prethfilter,tp,LOCATION_DECK,0,1,1,nil,c)
	Duel.SendtoExtraP(g,tp,REASON_EFFECT)
end
function cm.cfilter(c)
	return c:IsSetCard(0x303) 
end
function cm.prethfilter(c)
	return c:IsSetCard(0x303) and c:IsType(TYPE_RITUAL) and c:IsType(TYPE_MONSTER)
end
function cm.thfilter(c)
	return cm.prethfilter(c) and c:IsAbleToHand()
end
