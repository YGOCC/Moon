--Oscurità Irregolare - Shirley Isterica
...
function cid.filter(c,tp)
	return c:IsType(TYPE_XYZ) and c:IsAttribute(ATTRIBUTE_DARK) and c:IsControler(tp) and c:IsSummonType(SUMMON_TYPE_XYZ) and c:GetOverlayCount()>=5
end
function cid.scfilter(c)
	return c:IsSetCard(0x4999) and c:IsType(TYPE_MONSTER) and not c:IsCode(id) and c:IsAbleToHand()
end
function cid.cfilter(c)
	return c:IsFaceup() and c:IsSetCard(0x4999) and c:IsType(TYPE_MONSTER) and c:IsType(TYPE_XYZ)
end
--salvage
