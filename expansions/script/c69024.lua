...
function s.filter1(c,e,tp)
	local rk=c:GetRank()
	return rk>0 and c:IsFaceup() and c:IsSetCard(0x6969)
...
function s.filter2(c,e,tp,mc,rk)
	...
	return c:IsRank(rk) and c:IsSetCard(0x6969) and mc:IsCanBeXyzMaterial(c)
	...
	if sc then
		...
		if Duel.GetLP(tp)>Duel.GetLP(1-tp) and c:IsRelateToEffect(e) then
			...
			if Duel.GetTurnPlayer()~=tp then
				...
				Duel.Recover(p,sc:GetAttack(),REASON_EFFECT)
			end
		end
	end
end
