...
function c240100216.initial_effect(c)
	...
	e2:SetCondition(c240100216.spcon)
	e2:SetValue(c240100216.spval)
	c:RegisterEffect(e2)
	local e3=Effect.CreateEffect(c)
	e3:SetType(EFFECT_TYPE_SINGLE+EFFECT_TYPE_TRIGGER_F)
	e3:SetCode(EVENT_SPSUMMON_SUCCESS)
	e3:SetCountLimit(1,240100216)
	e3:SetProperty(EFFECT_FLAG_PLAYER_TARGET)
	e3:SetDescription(1108)
	e3:SetCondition(c240100216.mcon(c240100216.drcon))
	e3:SetTarget(c240100216.drtg)
	e3:SetOperation(c240100216.drop)
	c:RegisterEffect(e3)
	local o3=e3:Clone()
	o3:SetType(EFFECT_TYPE_SINGLE+EFFECT_TYPE_TRIGGER_O)
	o3:SetProperty(EFFECT_FLAG_PLAYER_TARGET+EFFECT_FLAG_DELAY)
	o3:SetCondition(c240100216.ocon(c240100216.drcon))
	c:RegisterEffect(o3)
end
function c240100216.spcon(e,c)
	if c==nil then return true end
	local tp=c:GetControler()
	local g=Duel.GetMatchingGroup(function(c) return c:IsFaceup() and c:IsRace(RACE_CYBERSE) and not c:IsCode(240100216) end,tp,LOCATION_MZONE,0,nil)
...
function c240100216.lmop(e,tp,eg,ep,ev,re,r,rp)
	...
	for tc in aux.Next(g) do
		...
		e1:SetValue(c240100216.lmval(t[op]))
		e1:SetReset(RESET_EVENT+0x1fe0000)
		tc:RegisterEffect(e1)
	end
end
function c240100216.lmval(t)
	return  function(e,c)
...
function c240100216.drcon(e)
	return e:GetHandler():GetSummonType()==SUMMON_TYPE_SPECIAL+1
end
function c240100216.drtg(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return e:IsHasType(EFFECT_TYPE_TRIGGER_F) or Duel.IsPlayerCanDraw(tp,1) end
	Duel.SetTargetPlayer(tp)
	Duel.SetTargetParam(1)
	Duel.SetOperationInfo(0,CATEGORY_DRAW,nil,0,tp,1)
end
function c240100216.drop(e,tp,eg,ep,ev,re,r,rp)
