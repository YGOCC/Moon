...
--    return Duel.IsExistingMatchingCard(card.filter,tp,LOCATION_GRAVE,0,1,nil,e,tp)
--end
...
function card.sptg(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
    if chkc then return chkc:IsLocation(LOCATION_GRAVE) and card.filter(chkc,e,tp) end
