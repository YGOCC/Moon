...
--Filters
function cid.ritualmonsterfilter(c)
...
function cid.target(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	if chkc then return chkc:IsLocation(LOCATION_GRAVE) and chkc:IsControler(tp) and cid.ritualmonsterfilter(chkc) end
	if chk==0 then return Duel.IsExistingMatchingCard(cid.ritualmonsterfilter,tp,LOCATION_GRAVE,0,1,nil,tp) end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_ATOHAND)
	local g=Duel.SelectTarget(tp,cid.ritualmonsterfilter,tp,LOCATION_GRAVE,0,1,1,nil)
