...
--「S・VINE」ワイヴァーン
function c240100184.initial_effect(c)
	...
	e2:SetCondition(c240100184.spcon)
	e2:SetTarget(c240100184.sptg)
	e2:SetOperation(c240100184.spop)
	c:RegisterEffect(e2)
	...
	e0:SetCondition(c240100184.con)
	e0:SetTarget(c240100184.tg)
	e0:SetOperation(c240100184.op)
	c:RegisterEffect(e0)
end
function c240100184.spcon(e,tp,eg,ep,ev,re,r,rp)
	local c=e:GetHandler()
	return c:IsPreviousLocation(LOCATION_DECK+LOCATION_GRAVE+LOCATION_HAND) and c:IsFaceup()
end
function c240100184.sptg(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return true end
	Duel.SetOperationInfo(0,CATEGORY_SPECIAL_SUMMON,e:GetHandler(),1,0,0)
end
function c240100184.spop(e,tp,eg,ep,ev,re,r,rp)
	local c=e:GetHandler()
	if c:IsRelateToEffect(e) then
		Duel.SpecialSummon(c,1,tp,tp,false,false,POS_FACEUP_ATTACK)
	end
end
function c240100184.con(e,tp,eg,ep,ev,re,r,rp)
...
function c240100184.tg(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	...
	local g1=Duel.SelectTarget(tp,c240100184.filter,tp,LOCATION_REMOVED,0,1,2,nil)
	Duel.SetOperationInfo(0,CATEGORY_TOGRAVE,g1,g1:GetCount(),0,0)
end
function c240100184.op(e,tp,eg,ep,ev,re,r,rp)
