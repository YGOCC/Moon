...
function c249000825.cost(e,tp,eg,ep,ev,re,r,rp,chk)
	...
	if chk==0 then return Duel.GetCustomActivityCount(249000825,tp,ACTIVITY_SPSUMMON)==0 and (Duel.IsExistingMatchingCard(c249000825.costfilter,tp,LOCATION_GRAVE,0,1,nil)
...
function c249000825.operation(e,tp,eg,ep,ev,re,r,rp)
	...
	Duel.ConfirmCards(1-tp,g)
	if Duel.GetLocationCountFromEx(tp)>0 and Duel.IsExistingMatchingCard(c249000825.rmfilter,tp,LOCATION_MZONE+LOCATION_GRAVE+LOCATION_HAND,0,1,nil,sc:GetLink()+2)
		and Duel.SelectYesNo(tp,2) then
		local g2=Duel.SelectMatchingCard(tp,c249000825.rmfilter,tp,LOCATION_MZONE+LOCATION_GRAVE+LOCATION_HAND,0,1,1,sc:GetLink()+2)
