...
function c90000094.initial_effect(c)
	...
	e2:SetCountLimit(1,90000094)
	e2:SetTarget(c90000094.target2)
	e2:SetOperation(c90000094.operation2)
	c:RegisterEffect(e2)
	--Place
	...
	e3:SetCondition(c90000094.condition3)
	e3:SetOperation(c90000094.operation3)
	c:RegisterEffect(e3)
	--ATK Up
...
function c90000094.cost1(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.IsExistingMatchingCard(c90000094.filter1,tp,LOCATION_ONFIELD,0,1,nil) end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_DESTROY)
	local g=Duel.SelectMatchingCard(tp,c90000094.filter1,tp,LOCATION_ONFIELD,0,1,1,nil)
...
function c90000094.target2(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.IsExistingMatchingCard(c90000094.filter2,tp,LOCATION_SZONE,0,1,nil)
		and Duel.IsExistingTarget(Card.IsFaceup,tp,0,LOCATION_ONFIELD,1,nil) end
	local ct=Duel.GetMatchingGroupCount(c90000094.filter2,tp,LOCATION_SZONE,0,nil)
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_DESTROY)
...
function c90000094.condition3(e)
	local c=e:GetHandler()
	return c:IsFaceup() and c:IsLocation(LOCATION_MZONE) and c:IsReason(REASON_DESTROY)
end
function c90000094.operation3(e,tp,eg,ep,ev,re,r,rp)
	local c=e:GetHandler()
	local e1=Effect.CreateEffect(c)
...
function c90000094.value4(e,c)
	return Duel.GetMatchingGroupCount(c90000094.filter2,e:GetHandlerPlayer(),LOCATION_SZONE,0,nil)*300
end