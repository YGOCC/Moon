--The Weather Painter Ground
local card = c210310401
function card.initial_effect(c)
	...
	e1:SetTarget(card.sptg1)
	e1:SetOperation(card.spop1)
	c:RegisterEffect(e1)
	--special summon
	...
	e3:SetDescription(aux.Stringid(210310401,1))
...
function card.spreg(e,tp,eg,ep,ev,re,r,rp)
	...
	if c:IsReason(REASON_COST) and rc:IsSetCard(0x109) then
		...
		c:RegisterFlagEffect(210310401,RESET_EVENT+0x1fe0000+RESET_PHASE+PHASE_END,0,2)
	end
end
function card.spcon(e,tp,eg,ep,ev,re,r,rp)
	return e:GetLabelObject():GetLabel()==Duel.GetTurnCount() and e:GetHandler():GetFlagEffect(210310401)>0
end
function card.sptg(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.GetLocationCount(tp,LOCATION_MZONE)>0
		and e:GetHandler():IsCanBeSpecialSummoned(e,0,tp,false,false) end
	Duel.SetOperationInfo(0,CATEGORY_SPECIAL_SUMMON,e:GetHandler(),1,0,0)
	e:GetHandler():ResetFlagEffect(210310401)
end
