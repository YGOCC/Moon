...
function cod.fscon(e,tp,eg,ep,ev,re,r,rp)
	if eg:IsExists(cod.mfilter,1,nil) then
		if #eg==1 then
			e:SetLabelObject(eg:GetFirst())
			return true
		else
			local g=eg:Filter(cod.mfilter,nil)
			e:SetLabelObject(g)
			return true
		end
	else 
...
function cod.fstg(e,tp,eg,ep,ev,re,r,rp,chk)
	...
	if chk==0 then
        local chkf=tp
        local mg1=Duel.GetMatchingGroup(cod.filter0,tp,0,LOCATION_MZONE,nil)
        if mt==Group then
			mg1:Merge(gc)
		else
			mg1:AddCard(gc)
        ...
        if not res then
            ...
            if ce~=nil then
                ...
                res=Duel.IsExistingMatchingCard(cod.filter2,tp,LOCATION_EXTRA,0,1,nil,e,tp,mg2,mf,chkf)
            end
        end
        return res
    end
...
function cod.fsop(e,tp,eg,ep,ev,re,r,rp)
	...
	local mt=getmetatable(gc)
	local chkf=tp
    local mg1=Duel.GetMatchingGroup(cod.filter1,tp,0,LOCATION_MZONE,nil,e)
    if mt==Group then
		mg1:Merge(gc)
	else
		mg1:AddCard(gc)
    ...
    if ce~=nil then
        ...
        mg2=fgroup(ce,e,tp)
        local mf=ce:GetValue()
        sg2=Duel.GetMatchingGroup(cod.filter2,tp,LOCATION_EXTRA,0,nil,e,tp,mg2,mf,chkf)
    end
