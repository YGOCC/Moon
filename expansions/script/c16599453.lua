...
function c16599453.scfilter(c,code)
	return c:IsSetCard(0x1559) and c:IsType(TYPE_MONSTER) and c:IsAbleToHand() and c:GetCode()~=code and (c:IsLocation(LOCATION_DECK) or (c:IsFaceup() and c:IsLocation(LOCATION_REMOVED)))
end
...
function c16599453.sccost(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.IsExistingMatchingCard(c16599453.costfilter,tp,LOCATION_DECK,0,1,e:GetHandler(),e:GetHandler():GetLevel()) end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_REMOVE)
	local g=Duel.SelectMatchingCard(tp,c16599453.costfilter,tp,LOCATION_DECK,0,1,1,e:GetHandler(),e:GetHandler():GetLevel())
