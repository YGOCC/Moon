	...
	if chk==0 then
		if other>0 then
			if true then return Duel.GetLocationCountFromEx(tp)>0 and Duel.GetLocationCount(tp,LOCATION_MZONE)>0 and
				...
				Duel.IsExistingMatchingCard(cid.exfilter,tp,LOCATION_EXTRA,0,1,nil,e,tp,id+10)) or
				Duel.IsPlayerCanSpecialSummonMonster(tp,id+11,0x617,0x21+TYPE_XYZ,2600,1900,3,0x2000,0x10,0x5,tp,SUMMON_TYPE_XYZ) and
				Duel.IsExistingMatchingCard(cid.exfilter,tp,LOCATION_EXTRA,0,1,nil,e,tp,id+11))
			end
		else
			return Duel.GetLocationCountFromEx(tp)>0 and Duel.GetLocationCount(tp,LOCATION_MZONE)>0 and
				Duel.IsPlayerCanSpecialSummonMonster(tp,id+10,0x617,0x21+TYPE_XYZ,1800,1300,3,0x2000,0x10,0x5,tp,SUMMON_TYPE_XYZ) and
				Duel.IsExistingMatchingCard(cid.exfilter,tp,LOCATION_EXTRA,0,1,nil,e,tp,id+10) and
...
function cid.spop(e,tp,eg,ep,ev,re,r,rp)
	...
	if sg:GetCount()>0 and Duel.SpecialSummon(sg,0,tp,tp,false,false,POS_FACEUP)>0 and c:IsRelateToEffect(e) then
		...
		if other>0 then
			...
			fc=Duel.SelectMatchingCard(tp,cid.exfilter,tp,LOCATION_EXTRA,0,1,1,nil,e,tp,id+10):GetFirst()
		end
		...
		elseif code==id+11 then
			g=Group.FromCards(c,sc)
			local tg=Duel.GetMatchingGroup(cid.f,tp,LOCATION_MZONE,0,nil,g):Select(tp,1,1,nil)
			if tg then g:AddCard(tg:GetFirst()) end
		end
		Duel.XyzSummon(tp,fc,g)
	end
end
function cid.f(c,g) return c:IsFaceup() and c:IsSetCard(0x617) and not g:IsContains(c) and c:GetLevel()==3 end