...
function c88890004.initial_effect(c)
    ...
    e3:SetDescription(aux.Stringid(88890004,1))
    e3:SetType(EFFECT_TYPE_FIELD+EFFECT_TYPE_CONTINUOUS)
    e3:SetProperty(EFFECT_FLAG_CANNOT_DISABLE+EFFECT_FLAG_UNCOPYABLE)
    e3:SetCode(EVENT_PHASE+PHASE_STANDBY)
    e3:SetRange(LOCATION_MZONE)
    e3:SetCountLimit(1)
    e3:SetCondition(c88890004.paycon)
    e3:SetOperation(c88890004.payop)
    c:RegisterEffect(e3)
    --(4) To hand
    local e4=Effect.CreateEffect(c)
    e4:SetDescription(aux.Stringid(88890004,2))
    e4:SetCategory(CATEGORY_TOHAND+CATEGORY_SEARCH)
    e4:SetType(EFFECT_TYPE_IGNITION)
    e4:SetRange(LOCATION_HAND)
    e4:SetCost(c88890004.thcost)
    e4:SetTarget(c88890004.thtg)
    e4:SetOperation(c88890004.thop)
    c:RegisterEffect(e4)
    --(5) Place in S/T Zone
...
function c88890004.payop(e,tp,eg,ep,ev,re,r,rp)
    ...
    if Duel.CheckLPCost(tp,500) and Duel.SelectYesNo(tp,aux.Stringid(88890004,1)) then
