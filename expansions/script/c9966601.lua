--Oscurità Irregolare - Medico Maledetto
...
function cid.tfilter(c,tp)
	return c:IsLocation(LOCATION_MZONE) and c:IsControler(tp) and c:IsFaceup() and c:IsSetCard(0x4999) and c:IsType(TYPE_XYZ)
end
...
function cid.xyzfilter(c)
	return c:IsSetCard(0x4999) and c:IsType(TYPE_MONSTER) and (c:IsLocation(LOCATION_GRAVE) or (c:IsLocation(LOCATION_REMOVED) and c:IsFaceup()))
end
--unaffected
...
function cid.attachcon(e,tp,eg,ep,ev,re,r,rp)
	local c=e:GetHandler()
	return c:IsSetCard(0x4999) and c:IsType(TYPE_XYZ) and c:IsType(TYPE_MONSTER)
