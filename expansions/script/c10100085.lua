...
function c10100085.initial_effect(c)
	...
	e4:SetCountLimit(1)
	e4:SetCondition(c10100085.spcon)
	e4:SetTarget(c10100085.eqtg)
...
function c10100085.eqfilter2(c)
	return c:IsFaceup() and c:IsSetCard(0x326) and c:GetLevel()==2
end
function c10100085.eqtg(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	if chkc then return chkc:IsLocation(LOCATION_MZONE) and chkc:IsControler(tp) and c10100085.eqfilter2(chkc) end
	if chk==0 then return e:GetHandler():IsRelateToEffect(e) and Duel.GetLocationCount(tp,LOCATION_SZONE)>0
		and Duel.IsExistingTarget(c10100085.eqfilter2,tp,LOCATION_MZONE,0,1,nil) end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_EQUIP)
	Duel.SelectTarget(tp,c10100085.eqfilter2,tp,LOCATION_MZONE,0,1,1,nil)
	Duel.SetOperationInfo(0,CATEGORY_EQUIP,e:GetHandler(),1,0,0)
end
function c10100085.operation(e,tp,eg,ep,ev,re,r,rp)
	local c=e:GetHandler()
