--Oscurità Irregolare - Doreen, il Perforatore
...
function cid.cfilter(c)
	return c:IsFaceup() and c:IsSetCard(0x4999) and c:IsType(TYPE_XYZ) and c:IsType(TYPE_MONSTER)
end
function cid.atkfilter(c)
	return c:IsFaceup() and c:IsType(TYPE_XYZ) and c:IsSetCard(0x4999) and c:GetOverlayCount()>0
end
--atk boost
function cid.atktg(e,c)
	return c:IsType(TYPE_XYZ) and c:IsSetCard(0x4999) and c:GetOverlayCount()>0
end
