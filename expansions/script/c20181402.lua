...
function cid.initial_effect(c)
	...
	e3:SetCondition(function(e) return c:IsSummonType(SUMMON_TYPE_LINK) end)
	e3:SetCost(cid.cost)
...
function cid.thtg(e,tp,eg,ep,ev,re,r,rp,chk)
	...
	if chk==0 then return Duel.IsExistingMatchingCard(cid.filter,tp,LOCATION_DECK,0,1,nil) and c:IsAbleToHand() end
...
function cid.activate(e,tp,eg,ep,ev,re,r,rp)
	...
	local tc=g:GetFirst()
	if tc and Duel.SendtoGrave(g,REASON_EFFECT) and tc:IsLocation(LOCATION_GRAVE) then
		Duel.BreakEffect()
