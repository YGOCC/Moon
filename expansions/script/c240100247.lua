...
function c240100247.initial_effect(c)
	...
	e1:SetTarget(c240100247.destg)
	e1:SetOperation(c240100247.desop)
	c:RegisterEffect(e1)
	local e4=Effect.CreateEffect(c)
	e4:SetType(EFFECT_TYPE_FIELD+EFFECT_TYPE_TRIGGER_O)
	e4:SetCode(EVENT_BE_BATTLE_TARGET)
	e4:SetRange(LOCATION_MZONE)
	e4:SetCondition(c240100247.discon)
	e4:SetTarget(c240100247.tg)
	e4:SetOperation(c240100247.op)
	c:RegisterEffect(e4)
end
function c240100247.destg(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
...
function c240100247.desop(e,tp,eg,ep,ev,re,r,rp)
	local tc=Duel.GetFirstTarget()
	if tc:IsRelateToEffect(e) then
		Duel.Remove(tc,POS_FACEUP,REASON_EFFECT)
	end
end
function c240100247.discon(e,tp,eg,ep,ev,re,r,rp)
	local ec=e:GetHandler()
	return ec==Duel.GetAttackTarget() or ec==Duel.GetAttacker() and ec:GetBattleTarget()
end
function c240100247.tg(e,tp,eg,ep,ev,re,r,rp,chk)
	local bc=e:GetHandler():GetBattleTarget()
	if chk==0 then return bc:IsControler(1-tp) and bc:IsControlerCanBeChanged() end
	Duel.SetTargetCard(bc)
end
function c240100247.op(e,tp,eg,ep,ev,re,r,rp)
	...
	if c:IsRelateToEffect(e) and tc:IsRelateToEffect(e) then
		...
		e3:SetOperation(c240100247.psop)
		e3:SetReset(RESET_PHASE+PHASE_END,1)
		Duel.RegisterEffect(e3,tp)
	end
end
function c240100247.psop(e,tp,eg,ep,ev,re,r,rp)
