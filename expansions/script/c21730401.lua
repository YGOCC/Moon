...
--add from deck to hand
function c21730401.filter(c)
	return c:IsSetCard(0x719)
end
function c21730401.thfilter(c)
...
function c21730401.cost(e,tp,eg,ep,ev,re,r,rp,chk)
	...
	local b1=Duel.CheckReleaseGroup(tp,c21730401.filter,1,false,nil,nil,tp)
	...
	if b2 and (not b1 or Duel.SelectYesNo(tp,aux.Stringid(21730411,1))) then
		...
		local g=Duel.SelectReleaseGroup(tp,c21730401.filter,1,1,false,nil,nil,tp)
		Duel.Release(g,REASON_COST)
	end
end
function c21730401.target(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.IsExistingMatchingCard(c21730401.thfilter,tp,LOCATION_DECK,0,1,nil) end
	Duel.SetOperationInfo(0,CATEGORY_TOHAND,nil,1,tp,LOCATION_DECK)
end
function c21730401.operation(e,tp,eg,ep,ev,re,r,rp)
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_ATOHAND)
	local g=Duel.SelectMatchingCard(tp,c21730401.thfilter,tp,LOCATION_DECK,0,1,1,nil)
