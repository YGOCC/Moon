...
function cid.sfilter1(c,e,tp)
	local lv=c:GetLevel()-e:GetHandler():GetSynchroLevel()
	...
	return c:IsType(TYPE_SYNCHRO) and lv>0 and c:IsSetCard(0xc97) and c:IsCanBeSpecialSummoned(e,SUMMON_TYPE_SYNCHRO,tp,false,false)
		and rg:CheckWithSumEqual(Card.GetSynchroLevel,rlv,1,63)
end
function cid.sfilter2(c)
	return c:GetSynchroLevel()>0 and not c:IsType(TYPE_TUNER) and c:IsAbleToRemove()
end
...
function cid.sop(e,tp,eg,ep,ev,re,r,rp)
	...
	local lv=g1:GetFirst():GetLevel()-c:GetSynchroLevel()
	local rg=Duel.GetMatchingGroup(cid.sfilter2,tp,LOCATION_HAND+LOCATION_GRAVE,0,nil)
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_REMOVE)
	local g2=rg:SelectWithSumEqual(tp,Card.GetSynchroLevel,lv,1,63)
