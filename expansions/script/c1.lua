...
function c1.initial_effect(c)
	...
	e2:SetCountLimit(1)
	e2:SetCondition(c1.condition)
	e2:SetCost(c1.cost)
	e2:SetOperation(c1.operation)
	c:RegisterEffect(e2)
end
function c1.sumop(e,tp,eg,ep,ev,re,r,rp)
	if Duel.GetFlagEffect(tp,1)~=0 then return end
...
function c1.condition(e,tp,eg,ep,ev,re,r,rp)
	return Duel.GetTurnPlayer()~=tp and (Duel.GetCurrentPhase()>=PHASE_BATTLE_START and Duel.GetCurrentPhase()<=PHASE_BATTLE)
end
function c1.cost(e,tp,eg,ep,ev,re,r,rp,chk)
