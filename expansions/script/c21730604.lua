...
function s.initial_effect(c)
	...
	e3:SetTarget(s.add_tg)
	e3:SetOperation(s.add_op)
	c:RegisterEffect(e3)
	--summon
...
function s.add_tg(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	if chkc then return chkc:IsLocation(LOCATION_GRAVE) and chkc:IsControler(tp) and s.add_filter(chkc) end
	if chk==0 then return Duel.IsExistingTarget(s.add_filter,tp,LOCATION_GRAVE,0,1,nil) end
