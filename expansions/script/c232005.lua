...
function c232005.dfilter(c,lv)
	return c:IsFaceup()
end
function c232005.sptg1(e,tp,eg,ep,ev,re,r,rp,chk)
   if chkc then return chkc:IsControler(1-tp) and chkc:IsLocation(LOCATION_MZONE) and c232005.dfilter(chkc,e:GetLabel()) end
	if chk==0 then return true end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_DESTROY)
	local g=Duel.SelectTarget(tp,c232005.dfilter,tp,0,LOCATION_MZONE,1,1,nil,e:GetLabel())
