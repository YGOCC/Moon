...
function c249000655.tgfilter3(c,e,tp,rc)
	return c:IsSetCard(0x1052) and c:IsRace(rc) and c:IsCanBeSpecialSummoned(e,0,tp,true,false)
end
	...
	if tc then
			...
			if sc then
				...
				if Duel.SpecialSummonStep(sc,0,tp,tp,true,false,POS_FACEUP) then
					...
					e3:SetCountLimit(1)
					e3:SetCondition(c249000655.retcon)
					e3:SetOperation(c249000655.retop)
					e3:SetLabel(1)
