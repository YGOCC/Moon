...
function c515171116.initial_effect(c)
    ...
    e2:SetCondition(c515171116.actcon)
    c:RegisterEffect(e2)
end
...
function c515171116.target(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
    ...
    if chk==0 then
        ...
        e:SetLabel(0)
        return Duel.IsExistingMatchingCard(c515171116.filter,tp,LOCATION_HAND+LOCATION_DECK,0,1,nil) 
    end
    e:SetLabel(0)
    Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_SELECT)
    local g=Duel.SelectMatchingCard(tp,c515171116.filter,tp,LOCATION_HAND+LOCATION_DECK,0,1,1,nil)
