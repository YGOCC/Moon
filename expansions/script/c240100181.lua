...
function c240100181.initial_effect(c)
	...
	e1:SetCondition(c240100181.spcon)
	e1:SetTarget(c240100181.sptg)
	e1:SetOperation(c240100181.spop)
	c:RegisterEffect(e1)
	local e2=e1:Clone()
	...
	e3:SetTarget(c240100181.target)
	e3:SetOperation(c240100181.operation)
	c:RegisterEffect(e3)
end
function c240100181.cfilter(c)
	return c:IsFaceup() and c:GetLevel()==4
end
function c240100181.spcon(e,tp,eg,ep,ev,re,r,rp)
	return eg:IsExists(c240100181.cfilter,1,nil)
end
function c240100181.sptg(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.GetLocationCount(tp,LOCATION_MZONE)>0
		and e:GetHandler():IsCanBeSpecialSummoned(e,0,tp,false,false) end
	Duel.SetOperationInfo(0,CATEGORY_SPECIAL_SUMMON,e:GetHandler(),1,0,0)
end
function c240100181.spop(e,tp,eg,ep,ev,re,r,rp)
...
function c240100181.target(e,tp,eg,ep,ev,re,r,rp,chk)
	...
	if chk==0 then return c:IsAbleToGrave()
		and Duel.IsExistingMatchingCard(c240100181.filter,tp,LOCATION_REMOVED+LOCATION_DECK,0,1,nil) end
	Duel.SetOperationInfo(0,CATEGORY_TOGRAVE,c,1,0,0)
	Duel.SetOperationInfo(0,CATEGORY_TOHAND,nil,1,tp,LOCATION_REMOVED+LOCATION_DECK)
end
function c240100181.operation(e,tp,eg,ep,ev,re,r,rp)
	local c=e:GetHandler()
	...
	local g=Duel.SelectMatchingCard(tp,c240100181.filter,tp,LOCATION_REMOVED+LOCATION_DECK,0,1,1,nil)
