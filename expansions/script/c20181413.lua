...
function cid.cfilter(c)
	return c:IsAbleToGraveAsCost() and c:IsSetCard(0x9ba) and c:IsType(TYPE_MONSTER) and (c:IsLocation(LOCATION_HAND) or (c:IsFaceup() and c:IsType(TYPE_PENDULUM)))
end
...
function cid.filter(c)
	return c:IsSetCard(0x9ba) and c:IsType(TYPE_MONSTER) and c:IsAbleToHand()
