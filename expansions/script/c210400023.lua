...
function c210400023.initial_effect(c)
	...
	ss:SetCondition(c210400023.con1)
	ss:SetTarget(c210400023.target1)
	ss:SetOperation(c210400023.op1)
	c:RegisterEffect(ss)
	local sl=ss:Clone()
	sl:SetCategory(CATEGORY_SPECIAL_SUMMON)
	sl:SetProperty(EFFECT_FLAG_DAMAGE_STEP)
	sl:SetCountLimit(1,210400023)
	sl:SetCondition(c210400023.con2)
	sl:SetTarget(c210400023.target2)
	sl:SetOperation(c210400023.op2)
	c:RegisterEffect(sl)
end
...
function c210400023.target1(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	if chkc then return chkc:IsControler(tp) and chkc:IsLocation(LOCATION_REMOVED) and c210400023.filter(chkc,e,tp) end
	if chk==0 then return true end
	local c=e:GetHandler()
	local ct=e:GetLabel()
	local g=Duel.SelectMatchingCard(tp,c210400023.filter,tp,LOCATION_REMOVED,0,ct,ct,nil,e,tp)
...
function c210400023.con2(e,tp,eg,ep,ev,re,r,rp)
	local c=e:GetHandler()
	return c:IsPreviousLocation(LOCATION_GRAVE) and c:IsFaceup()
end
function c210400023.target2(e,tp,eg,ep,ev,re,r,rp,chk)
	local c=e:GetHandler()
	if chk==0 then return true end
	Duel.SetOperationInfo(0,CATEGORY_SPECIAL_SUMMON,c,1,0,0)
end
function c210400023.op2(e,tp,eg,ep,ev,re,r,rp)
	local c=e:GetHandler()
