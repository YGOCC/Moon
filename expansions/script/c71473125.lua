...
function c71473125.initial_effect(c)
    ...
    e7:SetTarget(c71473125.syntg)
    e7:SetValue(1)
    e7:SetOperation(c71473125.synop)
    c:RegisterEffect(e7)
end
...
function c71473125.filter1(c,e,tp)
...
function c71473125.sptg(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
    if chkc then return chkc:IsLocation(LOCATION_PZONE) and chkc:IsControler(tp) and c71473125.filter1(chkc,e,tp) end
    if chk==0 then return Duel.GetLocationCount(tp,LOCATION_MZONE)>0
        and Duel.IsExistingTarget(c71473125.filter1,tp,LOCATION_PZONE,0,1,nil,e,tp) end
    Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_SPSUMMON)
    local g=Duel.SelectTarget(tp,c71473125.filter1,tp,LOCATION_PZONE,0,1,1,nil,e,tp)
...
c71473125.tuner_filter=aux.FilterBoolFunction(Card.IsSetCard,0x1C1D)
function c71473125.synfilter(c,syncard,tuner,f)
    return c:IsFaceup() and c:IsNotTuner() and c:IsCanBeSynchroMaterial(syncard,tuner) and c:IsSetCard(0x1C1D) and (f==nil or f(c))
end
function c71473125.syntg(e,syncard,f,minc,maxc)
    local c=e:GetHandler()
    local lv=syncard:GetLevel()-c:GetLevel()
    if lv<=0 then return false end
    local g=Duel.GetMatchingGroup(c71473125.synfilter,syncard:GetControler(),LOCATION_MZONE,LOCATION_MZONE,c,syncard,c,f)
    local res=g:CheckWithSumEqual(Card.GetSynchroLevel,lv,minc,maxc,syncard)
    return res
end
function c71473125.synop(e,tp,eg,ep,ev,re,r,rp,syncard,f,minc,maxc)
    local c=e:GetHandler()
    local lv=syncard:GetLevel()-c:GetLevel()
    local g=Duel.GetMatchingGroup(c71473125.synfilter,syncard:GetControler(),LOCATION_MZONE,LOCATION_MZONE,c,syncard,c,f)
