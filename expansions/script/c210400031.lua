...
function c210400031.initial_effect(c)
	...
	ae1:SetTarget(c210400031.reptg)
	ae1:SetOperation(c210400031.repop)
	...
	ae2:SetCountLimit(1)
	ae2:SetCost(c210400031.cost)
	ae2:SetOperation(c210400031.op)
...
function c210400031.acop(e,tp,eg,ep,ev,re,r,rp)
	...
	if eg:IsExists(c210400031.cfilter,1,c) then
		c:AddCounter(0x1,1)
		if c:GetCounter(0x1)==3 then
			Duel.RaiseSingleEvent(c,EVENT_CUSTOM+210400031,e,0,0,tp,0)
		end
	end
end
...
function c210400031.tg(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.GetMatchingGroupCount(Card.IsAbleToRemove,tp,0,LOCATION_EXTRA,nil)>0
		and Duel.IsExistingMatchingCard(c210400031.filter,tp,LOCATION_REMOVED,0,1,nil) end
	Duel.SetOperationInfo(0,CATEGORY_REMOVE,nil,1,1-tp,LOCATION_EXTRA)
end
function c210400031.op(e,tp,eg,ep,ev,re,r,rp)
	...
	if Duel.Remove(tc,POS_FACEUP,REASON_EFFECT)~=0 and tc:GetFirst():IsLevelAbove(1) and Duel.GetLocationCount(tp,LOCATION_MZONE)>0 then
		...
		local g=Duel.SelectMatchingCard(tp,c210400031.filter,tp,LOCATION_REMOVED,LOCATION_REMOVED,1,1,nil,e,tp)
