...
function c210400032.initial_effect(c)
	c:EnableReviveLimit()
	aux.AddSpatialProc(c,4,true,aux.FilterBoolFunction(Card.IsAttribute,ATTRIBUTE_WATER),aux.FilterBoolFunction(Card.IsAttribute,ATTRIBUTE_WATER))
...
function c210400032.target(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	...
	if chk==0 then
		...
		e:SetLabel(0)
		return Duel.IsExistingMatchingCard(c210400032.cfilter,tp,LOCATION_REMOVED,0,1,nil,tp)
	end
	local g=Duel.SelectMatchingCard(tp,c210400032.cfilter,tp,LOCATION_REMOVED,0,1,1,nil,tp)
	Duel.SendtoDeck(g,nil,2,REASON_COST)
	Duel.Hint(HINT_SELECTMSG,tp,aux.Stringid(210400032,0))
