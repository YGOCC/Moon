--Vigor of the Lost Earth
local card = c210310344
...
function card.filter1(c,tp)
	return c:IsCode(56494520) and c:GetActivateEffect():IsActivatable(tp)
end
function card.filter2(c)
	return c:IsType(TYPE_EQUIP) and c:IsCode(98374133) and Duel.IsExistingMatchingCard(card.eqcheck,tp,LOCATION_MZONE,0,1,nil,c)
end
...
function card.tdfilter(c)
	return c:IsCode(56494520,98374133) and c:IsAbleToRemoveAsCost()
end
