--Darkness of the Lost Mysticism
local card = c210310345
...
function card.filter1(c,tp)
	return c:IsCode(18161786) and c:GetActivateEffect():IsActivatable(tp)
end
function card.filter2(c)
	return c:IsType(TYPE_EQUIP) and c:IsCode(37120512) and Duel.IsExistingMatchingCard(card.eqcheck,tp,LOCATION_MZONE,0,1,nil,c)
end
...
function card.tdfilter(c)
	return c:IsCode(18161786,37120512) and c:IsAbleToRemoveAsCost()
end
