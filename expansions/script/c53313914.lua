...
function c53313914.rmtg(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.IsExistingMatchingCard(c53313914.rmfilter,tp,0,LOCATION_ONFIELD,1,nil,e) end
	local g=Duel.GetMatchingGroup(c53313914.rmfilter,tp,0,LOCATION_ONFIELD,nil,e)
...
function c53313914.rmop(e,tp,eg,ep,ev,re,r,rp)
	...
	if g:GetCount()>0 then
		...
		if e:GetHandler():IsHasEffect(53313927) and g:IsExists(Card.IsLocation,1,nil,LOCATION_MZONE)
			...
			fc:RegisterFlagEffect(53313927,RESET_EVENT+0x1fe0000+RESET_PHASE+PHASE_END,0,1)
		end
