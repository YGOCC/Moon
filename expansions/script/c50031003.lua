...
function c50031003.spfilter(c)
	return c:IsFaceup() and c:IsCode(500310030) and Duel.GetLocationCountFromEx(tp,tp,c)>0
end
function c50031003.hspcon(e,c)
  if c==nil then return true end
	...
	return Duel.GetLocationCount(tp,LOCATION_MZONE)>0
		and Duel.IsExistingMatchingCard(c50031003.spfilter,tp,LOCATION_MZONE,0,1,nil)
end
function c50031003.hspop(e,tp,eg,ep,ev,re,r,rp)
	   Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_MATERIAL)
	local g=Duel.SelectMatchingCard(tp,c50031003.spfilter,tp,LOCATION_MZONE,0,1,1,nil)
