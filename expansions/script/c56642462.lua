...
function c56642462.initial_effect(c)
    --special summon
    local e1=Effect.CreateEffect(c)
    e1:SetDescription(aux.Stringid(56642462,0))
    e1:SetCategory(CATEGORY_DESTROY+CATEGORY_SPECIAL_SUMMON)
    e1:SetType(EFFECT_TYPE_IGNITION)
    e1:SetRange(LOCATION_HAND+LOCATION_GRAVE)
    e1:SetCountLimit(1,56642462)
    e1:SetTarget(c56642462.sptg)
    e1:SetOperation(c56642462.spop)
    c:RegisterEffect(e1)
    --pop
    ...
    e2:SetCountLimit(1,56642562)
    e2:SetHintTiming(0,0x1e0)
    e2:SetTarget(c56642462.destg)
    e2:SetOperation(c56642462.desop)
    c:RegisterEffect(e2)
end
function c56642462.desfilter(c)
...
function c56642462.locfilter(c,tp)
    return c:IsLocation(LOCATION_SZONE) and c:IsControler(tp)
end
function c56642462.sptg(e,tp,eg,ep,ev,re,r,rp,chk)
    ...
    if chk==0 then return c:IsCanBeSpecialSummoned(e,0,tp,false,false)
        ...
        and (ft>0 or g:IsExists(c56642462.locfilter,-ft+1,nil,tp)) end
...
function c56642462.spop(e,tp,eg,ep,ev,re,r,rp)
    ...
    if ft<1 then
        g1=g:FilterSelect(tp,c56642462.locfilter,1,1,nil,tp)
    else
...
function c56642462.desfilter1(c)
    return c:IsFaceup() and c:IsSetCard(0x207a) and c:IsType(TYPE_SPELL)
end
function c56642462.destg(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
    if chkc then return false end
    if chk==0 then return Duel.IsExistingTarget(c56642462.desfilter1,tp,LOCATION_ONFIELD,0,1,nil)
        and Duel.IsExistingTarget(aux.TRUE,tp,0,LOCATION_ONFIELD,1,nil) end
    Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_DESTROY)
    local g1=Duel.SelectTarget(tp,c56642462.desfilter1,tp,LOCATION_ONFIELD,0,1,1,nil)
    Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_DESTROY)
    local g2=Duel.SelectTarget(tp,aux.TRUE,tp,0,LOCATION_ONFIELD,1,1,nil)
    g1:Merge(g2)
    Duel.SetOperationInfo(0,CATEGORY_DESTROY,g1,2,0,0)
end
function c56642462.desop(e,tp,eg,ep,ev,re,r,rp)
