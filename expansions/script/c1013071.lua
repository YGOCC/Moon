...
function cod.rfilter(c)
	return c:IsAbleToExtra() and c:IsFaceup()
end
function cod.reptg(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	if chkc then return chkc:IsLocation(LOCATION_PZONE) and chkc:IsControler(tp) and cod.rfilter(chkc) end
	if chk==0 then return Duel.IsExistingMatchingCard(cod.rfilter,tp,LOCATION_PZONE,0,1,nil) end
