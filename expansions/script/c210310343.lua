--Flames of the Lost Fire
local card = c210310343
...
--activate
--burning spear filter
function card.filter1(c,tp)
	return c:IsCode(19384334) and c:GetActivateEffect():IsActivatable(tp)
end
function card.filter2(c)
	return c:IsType(TYPE_EQUIP) and c:IsCode(18937875) and Duel.IsExistingMatchingCard(card.eqcheck,tp,LOCATION_MZONE,0,1,nil,c)
end
...
function card.tdfilter(c)
	return c:IsCode(19384334,18937875) and c:IsAbleToRemoveAsCost()
end
