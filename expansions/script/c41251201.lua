...
function cid.initial_effect(c)
	...
	e3:SetTarget(cid.dtg)
	e3:SetOperation(cid.dop)
	c:RegisterEffect(e3)
...
function cid.operation(e,tp,eg,ep,ev,re,r,rp)
	...
	local g=Duel.GetFieldGroup(tp,LOCATION_HAND,LOCATION_HAND)
	if #g>0 then Duel.Destroy(g:RandomSelect(tp,1),REASON_EFFECT) end
end
...
function cid.cfilter3(c,tp)
	return not c:IsType(TYPE_EFFECT) and c:IsRace(RACE_PLANT)
		and Duel.IsExistingMatchingCard(Card.IsFacedown,tp,LOCATION_ONFIELD,LOCATION_ONFIELD,1,c)
end
function cid.dcost(e,tp,eg,ep,ev,re,r,rp,chk)
	e:SetLabel(1)
	if chk==0 then return Duel.CheckReleaseGroup(tp,cid.cfilter3,1,nil,tp) end
	Duel.Release(Duel.SelectReleaseGroup(tp,cid.cfilter3,1,1,nil,tp),REASON_COST)
end
