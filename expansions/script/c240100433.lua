...
--S－VINEの騎士クライッシャ
c240100433.spt_other_space=240100436
function c240100433.initial_effect(c)
	...
	e3:SetCost(c240100433.descost)
	e3:SetTarget(c240100433.destg)
	e3:SetOperation(c240100433.desop)
	c:RegisterEffect(e3)
end
function c240100433.descost(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.IsExistingMatchingCard(Card.IsAbleToRemoveAsCost,tp,LOCATION_HAND,0,1,nil) end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_REMOVE)
	local g=Duel.SelectMatchingCard(tp,Card.IsAbleToRemoveAsCost,tp,LOCATION_HAND,0,1,1,nil)
	Duel.Remove(g,POS_FACEUP,REASON_COST)
end
function c240100433.destg(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
...
function c240100433.desop(e,tp,eg,ep,ev,re,r,rp)
	...
	if tc:IsRelateToEffect(e) and Duel.SendtoHand(tc,nil,REASON_EFFECT)~=0 and tc:IsPreviousPosition(POS_FACEUP) then
		...
		e1:SetCondition(c240100433.con)
		e1:SetOperation(c240100433.op)
		e1:SetReset(RESET_PHASE+PHASE_END,2)
		Duel.RegisterEffect(e1,tp)
	end
end
function c240100433.con(e,tp,eg,ep,ev,re,r,rp)
	local tc=e:GetLabelObject()
	return tc:GetFlagEffect(240100433)~=0 and Duel.GetTurnCount()~=tc:GetTurnID()
end
function c240100433.op(e,tp,eg,ep,ev,re,r,rp)
	local tc=e:GetLabelObject()
