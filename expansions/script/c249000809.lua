...
function c249000809.initial_effect(c)
	...
	e1:SetCountLimit(1)
	e1:SetCondition(c249000809.condition)
	e1:SetOperation(c249000809.operation)
	c:RegisterEffect(e1)
end
function c249000809.confilter(c)
	return c:IsSetCard(0x1F2) and c:IsType(TYPE_MONSTER) and (c:IsFaceup() or c:IsLocation(LOCATION_GRAVE))
end
function c249000809.condition(e,tp,eg,ep,ev,re,r,rp)
	return Duel.IsExistingMatchingCard(c249000809.confilter,tp,LOCATION_MZONE+LOCATION_GRAVE+LOCATION_REMOVED,0,1,nil)
end
function c249000809.rmfilter(c)
	return (c:IsLevelAbove(1) or c:GetRank() > 0) and c:IsAbleToRemove()
end
function c249000809.getlevelorrank(c)
	if c:GetLevel() > c:GetRank() then return c:GetLevel() else return c:GetRank() end
end
function c249000809.ctfilter(c)
	return c:IsSetCard(0x1F2) and c:IsType(TYPE_MONSTER) and (c:IsFaceup() or c:IsLocation(LOCATION_GRAVE))
end
function c249000809.operation(e,tp,eg,ep,ev,re,r,rp)
	...
	if g:CheckWithSumGreater(c249000809.getlevelorrank,lvrk*2,1,99,e:GetHandler()) and lvrk >=4 and lvrk <=9
		...
		local sg=g:SelectWithSumGreater(tp,c249000809.getlevelorrank,lvrk*2,1,99,e:GetHandler())
		...
		e1:SetValue(c249000809.damval)
		e1:SetReset(RESET_PHASE+PHASE_END)
		Duel.RegisterEffect(e1,tp)
		local g2=Duel.GetMatchingGroup(c249000809.ctfilter,tp,LOCATION_MZONE+LOCATION_GRAVE+LOCATION_REMOVED,0,nil)
...
function c249000809.damval(e,re,val,r,rp,rc)
	return val/2
end