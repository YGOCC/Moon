...
function c10100119.initial_effect(c)
	...
	e1:SetTarget(c10100119.target)
	e1:SetOperation(c10100119.operation)
	c:RegisterEffect(e1)
	--Atk
...
function c10100119.eqlimit(e,c)
	return c:IsSetCard(0x326) and c:GetLevel()==2
end
function c10100119.filter(c)
	return c:IsSetCard(0x326) and c:GetLevel()==2
end
function c10100119.target(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	if chkc then return chkc:IsLocation(LOCATION_MZONE) and c10100119.filter(chkc) end
	if chk==0 then return Duel.IsExistingTarget(c10100119.filter,tp,LOCATION_MZONE,LOCATION_MZONE,1,nil) end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_EQUIP)
	Duel.SelectTarget(tp,c10100119.filter,tp,LOCATION_MZONE,LOCATION_MZONE,1,1,nil)
