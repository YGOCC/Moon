...
function c90000069.initial_effect(c)
	...
	e4:SetCountLimit(1)
	e4:SetTarget(c90000069.target4)
	e4:SetOperation(c90000069.operation4)
	c:RegisterEffect(e4)
end
function c90000069.target1(e,c,sump,sumtype,sumpos,targetp)
...
function c90000069.filter4_1(tc,ec)
	return tc:IsFaceup() and ec:CheckEquipTarget(tc)
end
function c90000069.filter4_2(c)
	return c:IsType(TYPE_EQUIP) and c:GetEquipTarget()~=nil
		and Duel.IsExistingTarget(c90000069.filter4_1,0,LOCATION_MZONE,LOCATION_MZONE,1,c:GetEquipTarget(),c)
end
function c90000069.target4(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.IsExistingTarget(c90000069.filter4_2,tp,LOCATION_SZONE,LOCATION_SZONE,1,nil) end
	Duel.Hint(HINT_SELECTMSG,tp,aux.Stringid(90000069,0))
	local g=Duel.SelectTarget(tp,c90000069.filter4_2,tp,LOCATION_SZONE,LOCATION_SZONE,1,1,nil)
	local ec=g:GetFirst()
	e:SetLabelObject(ec)
	Duel.Hint(HINT_SELECTMSG,tp,aux.Stringid(90000069,1))
	local tc=Duel.SelectTarget(tp,c90000069.filter4_1,tp,LOCATION_MZONE,LOCATION_MZONE,1,1,ec:GetEquipTarget(),ec)
end
function c90000069.operation4(e,tp,eg,ep,ev,re,r,rp)
