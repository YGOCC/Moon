	...
	if chk==0 then
		...
		if not res then
			...
			if ce~=nil then
				...
				res=Duel.IsExistingMatchingCard(cod.filter2,tp,LOCATION_EXTRA,0,1,nil,e,tp,mg2,mf,c,chkf)
			end
		end
		return res
	end
...
function cod.thfilter(c)
	return c:IsSetCard(0xf05a) and c:IsType(TYPE_MONSTER) and c:IsAbleToHand() and c:GetCode()~=id
end
