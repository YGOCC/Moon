--Cloudian Altusylph
function c210310310.initial_effect(c)
	...
	e2:SetCondition(c210310310.sdcon)
	c:RegisterEffect(e2)
		--counter
	...
	e3:SetOperation(c210310310.addc)
	c:RegisterEffect(e3)
		--to hand
	...
	e5:SetCost(c210310310.cost)
	e5:SetTarget(c210310310.thtg1)
	e5:SetOperation(c210310310.thop1)
	c:RegisterEffect(e5)
	end
	function c210310310.cost(e,tp,eg,ep,ev,re,r,rp,chk)
	local c=e:GetHandler()
	if chk==0 then return c:IsDiscardable() end
	Duel.SendtoGrave(c,REASON_COST+REASON_DISCARD)
end
function c210310310.thcost(e,tp,eg,ep,ev,re,r,rp,chk)
...
function c210310310.thop1(e,tp,eg,ep,ev,re,r,rp)
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_ATOHAND)
	local g=Duel.SelectMatchingCard(tp,c210310310.thfilter1,tp,LOCATION_DECK,0,1,1,nil)
	if g:GetCount()>0 then
		Duel.SendtoHand(g,nil,REASON_EFFECT)
		Duel.ConfirmCards(1-tp,g)
	end
end
	function c210310310.sdcon(e)
	return e:GetHandler():IsPosition(POS_FACEUP_DEFENSE)
end
	function c210310310.cfilter(c)
	return c:IsFaceup() and c:IsSetCard(0x18)
end
function c210310310.addc(e,tp,eg,ep,ev,re,r,rp)
	if e:GetHandler():IsRelateToEffect(e) then
		local ct=Duel.GetMatchingGroupCount(c210310310.cfilter,tp,LOCATION_MZONE,LOCATION_MZONE,nil)
