...
function c249000024.initial_effect(c)
	...
	e5:SetCondition(c249000024.condition)
...
function c249000024.condition(e,tp,eg,ep,ev,re,r,rp)
	return e:GetHandler():GetOriginalAttribute()==ATTRIBUTE_LIGHT
end
...
function c249000024.operation(e,tp,eg,ep,ev,re,r,rp)
	...
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_SPSUMMON)
	local ac=Duel.AnnounceCardFilter(tp,ATTRIBUTE_LIGHT,OPCODE_ISATTRIBUTE,TYPE_XYZ,OPCODE_ISTYPE,OPCODE_AND,249000024,OPCODE_ISCODE,OPCODE_OR)
	sc=Duel.CreateToken(tp,ac)
	while not (sc:IsType(TYPE_XYZ) and (sc:GetRank() == tc:GetRank()+2)
		and sc:IsAttribute(ATTRIBUTE_LIGHT) and sc:IsCanBeSpecialSummoned(e,SUMMON_TYPE_XYZ,tp,true,false))
	do
		ac=Duel.AnnounceCardFilter(tp,ATTRIBUTE_LIGHT,OPCODE_ISATTRIBUTE,TYPE_XYZ,OPCODE_ISTYPE,OPCODE_AND,249000024,OPCODE_ISCODE,OPCODE_OR)
		sc=Duel.CreateToken(tp,ac)
		if ac==249000024 then return end
	end
