...
function c1020092.initial_effect(c)
	--pendulum
	aux.EnablePendulumAttribute(c)
	local e1=Effect.CreateEffect(c)
	e1:SetDescription(aux.Stringid(1020092,0))
	e1:SetCategory(CATEGORY_SPECIAL_SUMMON+CATEGORY_TOKEN)
	e1:SetType(EFFECT_TYPE_FIELD+EFFECT_TYPE_TRIGGER_O)
	e1:SetProperty(EFFECT_FLAG_DELAY+EFFECT_FLAG_DAMAGE_STEP)
	e1:SetCode(EVENT_RELEASE)
	e1:SetRange(LOCATION_PZONE)
	e1:SetCountLimit(1,1020092)
	e1:SetCondition(c1020092.tkcon)
	e1:SetTarget(c1020092.tktg)
	e1:SetOperation(c1020092.tkop)
	c:RegisterEffect(e1)
	--spsummon hand
	local e2=Effect.CreateEffect(c)
	e2:SetDescription(aux.Stringid(1020092,1))
	e2:SetCategory(CATEGORY_SPECIAL_SUMMON)
	e2:SetType(EFFECT_TYPE_IGNITION)
	e2:SetRange(LOCATION_MZONE)
	e2:SetCountLimit(1,1120091)
	e2:SetCost(c1020092.spcost)
	e2:SetTarget(c1020092.sptg)
	e2:SetOperation(c1020092.spop)
	c:RegisterEffect(e2)
	--damage
	local e3=Effect.CreateEffect(c)
	e3:SetDescription(aux.Stringid(1020092,2))
	e3:SetCategory(CATEGORY_DAMAGE)
	e3:SetType(EFFECT_TYPE_SINGLE+EFFECT_TYPE_TRIGGER_O)
	e3:SetProperty(EFFECT_FLAG_CARD_TARGET+EFFECT_FLAG_DAMAGE_STEP+EFFECT_FLAG_DELAY)
	e3:SetCode(EVENT_RELEASE)
	e3:SetCountLimit(1,1220091)
	e3:SetCondition(c1020092.thcon)
	e3:SetTarget(c1020092.thtg)
	e3:SetOperation(c1020092.thop)
	c:RegisterEffect(e3)
end
--filters
...
function c1020092.cfilter(c,e,tp)
	return c:IsType(TYPE_PENDULUM) and c:IsRace(RACE_CYBERSE) and c:IsFaceup()
		and Duel.IsExistingMatchingCard(c1020092.spfilter,tp,LOCATION_GRAVE,0,1,nil,c:GetCode(),e,tp)
...
function c1020092.tkcon(e,tp,eg,ep,ev,re,r,rp)
	return eg:IsExists(c1020092.tkfilter,1,nil,tp)
end
function c1020092.tktg(e,tp,eg,ep,ev,re,r,rp,chk)
...
function c1020092.tkop(e,tp,eg,ep,ev,re,r,rp)
...
--spsummon extra
function c1020092.spcost(e,tp,eg,ep,ev,re,r,rp,chk)
	e:SetLabel(100)
	if chk==0 then return true end
end
function c1020092.sptg(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then 
		if e:GetLabel()~=100 then return false end
		e:SetLabel(0)
		return Duel.CheckReleaseGroup(tp,c1020092.cfilter,1,e:GetHandler(),e,tp) 
	end
	e:SetLabel(0)
	local g=Duel.SelectReleaseGroup(tp,c1020092.cfilter,1,1,e:GetHandler(),e,tp)
