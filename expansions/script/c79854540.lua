...
function c79854540.initial_effect(c)
	...
	e1:SetOperation(c79854540.sop)
	e1:SetValue(aux.tgoval)
	c:RegisterEffect(e1)
	local e2=Effect.CreateEffect(c)
	e2:SetType(EFFECT_TYPE_SINGLE)
	e2:SetCode(EFFECT_MATERIAL_CHECK)
	e2:SetValue(c79854540.valcheck)
...
--Synchro as material
function c79854540.valcheck(e,c)
...
--Check that summon is correct
function c79854540.scon(e,tp,eg,ep,ev,re,r,rp)
	return e:GetHandler():IsSummonType(SUMMON_TYPE_SYNCHRO) and e:GetLabel()==1
end
function c79854540.sop(e,tp,eg,ep,ev,re,r,rp)
...
function c79854540.atktg(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	...
	local g=eg:Filter(Card.IsControler,1,nil,1-tp)
	Duel.SetOperationInfo(0,CATEGORY_ATKCHANGE+CATEGORY_DISABLE,g,g:GetCount(),0,0)
end
function c79854540.atkop(e,tp,eg,ep,ev,re,r,rp)
	if not e:GetHandler():IsRelateToEffect(e) then return end
	local g=eg:Filter(c79854540.atkfilter,nil,e,1-tp)
	if g:GetCount()>0 then
		local tc=g:GetFirst()
		while tc and tc:IsRelateToEffect(e) and tc:IsFaceup() and tc:GetAttack()>0 and not tc:IsImmuneToEffect(e) do
