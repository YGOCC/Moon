--Deepwood Carnivorous Flower
local voc=c1912
function c1912.initial_effect(c)
	--cannot tribute
	...
	e3:SetDescription(aux.Stringid(1912,0))
	e3:SetType(EFFECT_TYPE_TRIGGER_F+EFFECT_TYPE_SINGLE)
    e3:SetProperty(EFFECT_FLAG_CARD_TARGET+EFFECT_FLAG_DAMAGE_STEP+EFFECT_FLAG_DELAY)
    e3:SetCode(EVENT_LEAVE_FIELD)
	e3:SetCondition(voc.condition)
	e3:SetTarget(voc.target2)
	e3:SetOperation(voc.operation2)
	c:RegisterEffect(e3)
	--self destroy
...
function voc.condition(e,tp,eg,ep,ev,re,r,rp)
	return e:GetHandler():IsLocation(LOCATION_GRAVE) and not e:GetHandler():IsReason(REASON_RETURN)
end
...
function voc.target2(e,tp,eg,ep,ev,re,r,rp,chk)
    if chk==0 then return true end
    Duel.SetOperationInfo(0,CATEGORY_TODECK,e:GetHandler(),1,0,0)
end
function voc.operation2(e,tp,eg,ep,ev,re,r,rp)
...
function voc.damtg(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	if chk==0 then return Duel.IsExistingMatchingCard(voc.cfilter,tp,LOCATION_DECK,0,1,nil) end
	Duel.SetOperationInfo(0,CATEGORY_TOGRAVE,nil,1,tp,LOCATION_DECK)
end
function voc.damop(e,tp,eg,ep,ev,re,r,rp)
	local g=Duel.SelectMatchingCard(tp,voc.cfilter,tp,LOCATION_DECK,0,1,1,nil)
