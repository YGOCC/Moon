...
function cid.initial_effect(c)
	...
	e1:SetTarget(cid.bouncetg)
	e1:SetOperation(cid.bounceop)
	c:RegisterEffect(e1)
	...
	e1x:SetCondition(cid.bouncecon_quick)
	e1x:SetTarget(cid.bouncetg)
	e1x:SetOperation(cid.bounceop)
...
--Bounce (Operation)
function cid.bouncecon_quick(e,tp,eg,ep,ev,re,r,rp)
	return e:GetHandler():GetFlagEffect(id)>0
end
function cid.bouncetg(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	...
	local g1=Duel.SelectTarget(tp,cid.filter,tp,LOCATION_MZONE,0,1,1,nil)
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_RTOHAND)
	local g2=Duel.SelectTarget(tp,Card.IsAbleToHand,tp,0,LOCATION_MZONE,1,1,nil)
	g1:Merge(g2)
	Duel.SetOperationInfo(0,CATEGORY_TOHAND,g1,2,0,0)
end
function cid.bounceop(e,tp,eg,ep,ev,re,r,rp)
...
function cid.destg(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	if chkc then return chkc~=c and chkc:IsLocation(LOCATION_MZONE) and chkc:IsControler(1-tp) and cid.desfilter(chkc) end
	if chk==0 then return Duel.IsExistingTarget(cid.desfilter,tp,0,LOCATION_MZONE,1,nil) end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_DESTROY)
	local g=Duel.SelectTarget(tp,cid.desfilter,tp,0,LOCATION_MZONE,1,1,nil)
