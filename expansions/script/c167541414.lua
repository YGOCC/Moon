...
function cid.filter2(c,e,tp,m,f,chkf)
	return c:IsType(TYPE_FUSION) and c:IsRace(RACE_DINOSAUR+RACE_DRAGON+RACE_SEASERPENT+RACE_WYRM) and (not f or f(c))
		and c:IsCanBeSpecialSummoned(e,SUMMON_TYPE_FUSION,tp,false,false) and c:CheckFusionMaterial(m,nil,chkf)
end
function cid.filterG(c)
	return c:IsType(TYPE_MONSTER) and not c:IsType(TYPE_EFFECT) and c:IsCanBeFusionMaterial() and c:IsAbleToRemove()
end
----------
function cid.target(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then
		...
		local mg2=Duel.GetMatchingGroup(cid.filterG,tp,LOCATION_GRAVE,0,nil)
		mg1:Merge(mg2)
...
function cid.activate(e,tp,eg,ep,ev,re,r,rp)
	...
	local mg2=Duel.GetMatchingGroup(cid.filterG,tp,LOCATION_GRAVE,0,nil):Filter(cid.filter1,nil,e)
	mg1:Merge(mg2)
