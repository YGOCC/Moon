--Glint of the Lost Light
local card = c210310340
...
function card.filter1(c,tp)
	return c:IsCode(81777047) and c:GetActivateEffect():IsActivatable(tp)
end
function card.filter2(c)
	return c:IsType(TYPE_EQUIP) and c:IsCode(39897277) and Duel.IsExistingMatchingCard(card.eqcheck,tp,LOCATION_MZONE,0,1,nil,c)
end
...
function card.tdfilter(c)
	return c:IsCode(81777047,39897277) and c:IsAbleToRemoveAsCost()
end
