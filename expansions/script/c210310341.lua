--Zephyr of the Lost Winds
local card = c210310341
...
function card.filter1(c,tp)
	return c:IsCode(45778932) and c:GetActivateEffect():IsActivatable(tp)
end
function card.filter2(c)
	return c:IsType(TYPE_EQUIP) and c:IsCode(55321970) and Duel.IsExistingMatchingCard(card.eqcheck,tp,LOCATION_MZONE,0,1,nil,c)
end
...
function card.tdfilter(c)
	return c:IsCode(45778932,55321970) and c:IsAbleToRemoveAsCost()
end
