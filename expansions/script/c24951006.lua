...
function c24951006.condition(e,tp,eg,ep,ev,re,r,rp)
	return ((Duel.IsPlayerCanSpecialSummonMonster(tp,24951011,0,0x5F453A,1500,1500,4,RACE_SPELLCASTER,ATTRIBUTE_LIGHT) and Duel.GetLocationCount(tp,LOCATION_MZONE)>0) or (Duel.IsExistingMatchingCard(c24951006.negfilter,tp,LOCATION_ONFIELD,LOCATION_ONFIELD,1,e:GetHandler())))
end
...
function c24951006.operation(e,tp,eg,ep,ev,re,r,rp)
	...
	if (Duel.IsPlayerCanSpecialSummonMonster(tp,24951011,0,0x5F453A,1500,1500,4,RACE_SPELLCASTER,ATTRIBUTE_LIGHT) and Duel.GetLocationCount(tp,LOCATION_MZONE)>0) then
		selectoption2 = 1
	end
