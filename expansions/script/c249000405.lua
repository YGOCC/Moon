...
function c249000405.initial_effect(c)
	...
	e2:SetCondition(c249000405.spcon)
	c:RegisterEffect(e2)
	--to grave
	...
	e3:SetOperation(c249000405.regop)
	c:RegisterEffect(e3)
end
...
function c249000405.spcon(e,c)
	if c==nil then return true end
	return Duel.GetFieldGroupCount(c:GetControler(),LOCATION_MZONE,0)==0
		and	Duel.GetFieldGroupCount(c:GetControler(),0,LOCATION_MZONE)>0
		and Duel.GetLocationCount(c:GetControler(),LOCATION_MZONE)>0
end
function c249000405.regop(e,tp,eg,ep,ev,re,r,rp)
	local c=e:GetHandler()
	local e1=Effect.CreateEffect(c)
	...
	e1:SetCountLimit(1,249000405)
	e1:SetTarget(c249000405.thtg)
	e1:SetOperation(c249000405.thop)
	e1:SetReset(RESET_EVENT+0x1fe0000+RESET_PHASE+PHASE_END)
	c:RegisterEffect(e1)
end
function c249000405.filter(c)
	return c:IsAbleToHand() and ((c:IsSetCard(0x1B9) and not c:IsCode(249000405)) or c:IsCode(249000400))
end
function c249000405.thtg(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.IsExistingMatchingCard(c249000405.filter,tp,LOCATION_DECK,0,1,nil) end
	Duel.SetOperationInfo(0,CATEGORY_TOHAND,nil,1,tp,LOCATION_DECK)
end
function c249000405.thop(e,tp,eg,ep,ev,re,r,rp)
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_ATOHAND)
	local g=Duel.SelectMatchingCard(tp,c249000405.filter,tp,LOCATION_DECK,0,1,1,nil)
	if g:GetCount()>0 then
