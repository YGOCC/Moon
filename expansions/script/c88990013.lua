...
function cm.initial_effect(c)
    ...
    e2:SetCountLimit(1,88990113)
    e2:SetCost(cm.bthcost)
    e2:SetTarget(cm.bthtg)
    e2:SetOperation(cm.bthop)
    c:RegisterEffect(e2)
end
function cm.linkfilter(c,tp,c2)
    return c:GetCode()==88990006 and Duel.GetLocationCountFromEx(tp,tp,c)>0
end
function cm.linkcon(e,c)
    if c==nil then return true end
    local tp=c:GetControler()
    return Duel.IsExistingMatchingCard(cm.linkfilter,tp,LOCATION_MZONE,0,1,nil,tp,c)
end
function cm.linkop(e,tp,eg,ep,ev,re,r,rp,c)
    local g1=Duel.SelectMatchingCard(tp,cm.linkfilter,tp,LOCATION_MZONE,0,1,1,nil,c)
    c:SetMaterial(g1)
...
function cm.thfilter(c)
    return c:IsSetCard(0xffd) and c:GetLevel()<13 and c:IsAbleToHand()
end
