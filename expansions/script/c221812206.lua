...
function cid.filter(c,e,tp)
	return c:IsFaceup() and c:IsSetCard(0xa67) and c:IsDestructable()
		and Duel.IsExistingMatchingCard(cid.cfilter,tp,LOCATION_ONFIELD,LOCATION_ONFIELD,1,Group.FromCards(e:GetHandler(),c),c:GetColumnGroup())
end
function cid.target(e,tp,eg,ep,ev,re,r,rp,chk)
	local c=e:GetHandler()
	if chk==0 then return Duel.IsExistingTarget(cid.filter,tp,LOCATION_MZONE,0,1,nil,e,tp) end
