...
--return to hand
function c25386872.regop(e,tp,eg,ep,ev,re,r,rp)
	local c=e:GetHandler()
	local eqg=c:GetEquipGroup()
	local e1=Effect.CreateEffect(c)
	e1:SetDescription(aux.Stringid(25386872,1))
	e1:SetCategory(CATEGORY_TOHAND)
	e1:SetType(EFFECT_TYPE_FIELD+EFFECT_TYPE_TRIGGER_F)
	e1:SetRange(LOCATION_MZONE)
	e1:SetCountLimit(1)
	e1:SetCode(EVENT_PHASE+PHASE_END)
	e1:SetCondition(c25386872.mandatory)
	e1:SetTarget(c25386872.tgtg)
	e1:SetOperation(c25386872.tgop)
	e1:SetReset(RESET_EVENT+0xc6c0000+RESET_PHASE+PHASE_END)
	c:RegisterEffect(e1)
	local e2=Effect.CreateEffect(c)
	e2:SetDescription(aux.Stringid(25386872,1))
	e2:SetCategory(CATEGORY_TOHAND)
	e2:SetType(EFFECT_TYPE_FIELD+EFFECT_TYPE_TRIGGER_O)
	e2:SetRange(LOCATION_MZONE)
	e2:SetCountLimit(1)
	e2:SetCode(EVENT_PHASE+PHASE_END)
	e2:SetCondition(c25386872.optional)
	e2:SetTarget(c25386872.tgtg)
	e2:SetOperation(c25386872.tgop)
	e2:SetReset(RESET_EVENT+0xc6c0000+RESET_PHASE+PHASE_END)
	c:RegisterEffect(e2)
end
function c25386872.mandatory(e,tp,eg,ep,ev,re,r,rp)
	local c=e:GetHandler()
	local eqg=c:GetEquipGroup()
	return not eqg:IsExists(c25386872.eqopt,1,nil)
end
function c25386872.optional(e,tp,eg,ep,ev,re,r,rp)
	local c=e:GetHandler()
	local eqg=c:GetEquipGroup()
	return eqg:IsExists(c25386872.eqopt,1,nil)
end
function c25386872.tgtg(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return true end
	Duel.SetOperationInfo(0,CATEGORY_TOHAND,e:GetHandler(),1,0,0)
end
function c25386872.tgop(e,tp,eg,ep,ev,re,r,rp)
	local c=e:GetHandler()
