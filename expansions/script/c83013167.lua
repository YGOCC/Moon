...
function cod.initial_effect(c)
	...
	e6:SetCost(cod.eqcost)
	e6:SetTarget(cod.eqtg2)
	e6:SetOperation(cod.eqop)
	c:RegisterEffect(e6)
end
...
function cod.eqcost(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.IsExistingMatchingCard(cod.costfilter,tp,LOCATION_HAND,0,1,nil) end
	Duel.DiscardHand(tp,cod.costfilter,1,1,REASON_COST+REASON_DISCARD,nil)
end
...
function cod.eqtg2(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	...
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_EQUIP)
	local g=Duel.SelectTarget(tp,cod.eqfilter,tp,LOCATION_MZONE,0,1,1,nil)
	Duel.SetOperationInfo(0,CATEGORY_EQUIP,g,1,0,0)
