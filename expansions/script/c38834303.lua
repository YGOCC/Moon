...
function c38834303.activate(e,tp,eg,ep,ev,re,r,rp)
	...
	local count2=0
	while tc do
		count1=count1+tc:GetCounter(0x100e)
		count2=count2+tc:GetCounter(0x1019)
		tc:RemoveCounter(tp,0,0,0)
		tc=sg:GetNext()
	end
	if count1>0 then
		Duel.RaiseEvent(e:GetHandler(),EVENT_REMOVE_COUNTER+0x100e,e,REASON_EFFECT,tp,tp,count1)
	end
	if count2>0 then
		Duel.RaiseEvent(e:GetHandler(),EVENT_REMOVE_COUNTER+0x1019,e,REASON_EFFECT,tp,tp,count2)
	end
end
