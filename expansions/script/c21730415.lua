...
function c21730415.initial_effect(c)
	...
	e1:SetCountLimit(1,21730415+EFFECT_COUNT_CODE_OATH)
	e1:SetTarget(c21730415.rettg)
	e1:SetOperation(c21730415.retop)
	c:RegisterEffect(e1)
end
--return entire field to hand
function c21730415.retfilter(c)
	return c:IsSetCard(0x719) and c:IsDiscardable()
end
function c21730415.rettg(e,tp,eg,ep,ev,re,r,rp,chk)
	local c=e:GetHandler()
	if chk==0 then return Duel.IsExistingMatchingCard(c21730415.retfilter,tp,LOCATION_HAND,0,1,c)
...
function c21730415.retop(e,tp,eg,ep,ev,re,r,rp)
	if Duel.DiscardHand(tp,c21730415.retfilter,1,1,REASON_EFFECT+REASON_DISCARD,nil)~=0 then
