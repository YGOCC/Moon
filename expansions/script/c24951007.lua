...
function c24951007.condition(e,tp,eg,ep,ev,re,r,rp)
	return ((Duel.IsPlayerCanSpecialSummonMonster(tp,24951012,0,0x5F453A,1000,1000,1,RACE_SPELLCASTER,ATTRIBUTE_LIGHT) and Duel.GetLocationCount(tp,LOCATION_MZONE)>0) or (Duel.IsExistingMatchingCard(c24951007.negfilter,tp,LOCATION_ONFIELD,LOCATION_ONFIELD,1,nil)))
end
...
function c24951007.operation(e,tp,eg,ep,ev,re,r,rp)
	...
	if (Duel.IsPlayerCanSpecialSummonMonster(tp,24951012,0,0x5F453A,1000,1000,1,RACE_SPELLCASTER,ATTRIBUTE_LIGHT) and Duel.GetLocationCount(tp,LOCATION_MZONE)>0) then
		selectoption2 = 1
	end
	...
	if selectoption2 == 1 then
		...
		e1:SetCondition(c24951007.rmcon)
		e1:SetOperation(c24951007.rmop)
...
function c24951007.rmcon(e,tp,eg,ep,ev,re,r,rp)
	local tc2=e:GetLabelObject()
	if tc2:GetFlagEffect(24951007)~=0 then
...
function c24951007.rmop(e,tp,eg,ep,ev,re,r,rp)
	local tc=e:GetLabelObject()
	Duel.Remove(tc,POS_FACEUP,REASON_EFFECT)
end
function c24951007.immfilter(e,te,tp)
	if te:IsActiveType(TYPE_TRAP) and not te:GetHandler():GetControler(tp) == tp then 
		return true 
	end
end