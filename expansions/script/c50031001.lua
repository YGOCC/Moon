...
function cid.spfilter(c,ec,tp)
	return c:IsFaceup() and c:IsCode(500310010) and c:IsCanBeEvoluteMaterial(ec)
...
function cid.operation(e,tp,eg,ep,ev,re,r,rp)
	...
	if opt==1 then
		...
		local dg=Duel.SelectMatchingCard(tp,cid.rmfilter,tp,LOCATION_SZONE+LOCATION_GRAVE,LOCATION_SZONE+LOCATION_GRAVE,1,1,nil)
		Duel.Remove(dg,POS_FACEUP,REASON_EFFECT)
	end
	if opt==2 then
		Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_REMOVE)
		local dg=Duel.SelectMatchingCard(tp,cid.rmfilter2,tp,LOCATION_SZONE+LOCATION_GRAVE,LOCATION_SZONE+LOCATION_GRAVE,1,1,nil)
