...
function cid.thfilter(c)
	return c:IsSetCard(0x2595) and c:IsAttribute(ATTRIBUTE_LIGHT) and c:IsRace(RACE_SPELLCASTER) and c:IsAbleToHand()
end
...
function cid.tgfilter(c)
	return c:IsType(TYPE_MONSTER) and c:IsRace(RACE_SPELLCASTER) and c:IsAbleToGrave() and c:IsSetCard(0x2595)
		and (c:IsLocation(LOCATION_DECK) or c:IsFaceup())
end
function cid.scfilter(c)
	return c:IsSetCard(0x2595) and c:IsAbleToHand() and (c:IsLocation(LOCATION_GRAVE) or (c:IsLocation(LOCATION_REMOVED) and c:IsFaceup()))
end
function cid.sctg(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.IsExistingMatchingCard(cid.tgfilter,tp,LOCATION_DECK+LOCATION_MZONE,LOCATION_MZONE,1,nil)
		and Duel.IsExistingMatchingCard(cid.scfilter,tp,LOCATION_GRAVE+LOCATION_REMOVED,0,1,nil)
	end
...
function cid.scop(e,tp,eg,ep,ev,re,r,rp)
	...
	if #g>0 and Duel.SendtoGrave(g,REASON_EFFECT)~=0 and g:IsExists(Card.IsLocation,1,nil,LOCATION_GRAVE) then
		...
		local sg=Duel.SelectMatchingCard(tp,aux.NecroValleyFilter(cid.scfilter),tp,LOCATION_GRAVE+LOCATION_REMOVED,0,1,1,nil)
		if #sg>0 then
...
function cid.confilter(c)
	return c:IsFaceup() and c:IsType(TYPE_MONSTER) and c:IsAttribute(ATTRIBUTE_LIGHT) and c:IsRace(RACE_SPELLCASTER)
end
function cid.negcon(e,tp,eg,ep,ev,re,r,rp)
	...
	return rp==1-tp and not e:GetHandler():IsStatus(STATUS_BATTLE_DESTROYED) and Duel.IsChainNegatable(ev) and re:GetActiveType()==0x4
end
