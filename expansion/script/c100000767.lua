...
function c100000767.initial_effect(c)
c:EnableUnsummonable()
	...
	e2:SetTarget(c100000767.splimit)
	c:RegisterEffect(e2)
	--special summon
	local e1=Effect.CreateEffect(c)
	e1:SetDescription(aux.Stringid(100000767,0))
	e1:SetCategory(CATEGORY_SPECIAL_SUMMON)
	e1:SetType(EFFECT_TYPE_QUICK_F)
	e1:SetRange(LOCATION_DECK)
	e1:SetCode(EVENT_CHAINING)
	e1:SetCondition(c100000767.spcon)
	e1:SetTarget(c100000767.sptg)
	e1:SetOperation(c100000767.spop)
	e1:SetCountLimit(1,100000767)	
	c:RegisterEffect(e1)
		--cannot release
...
function c100000767.sptg(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.GetLocationCount(tp,LOCATION_MZONE)>0
		and e:GetHandler():IsCanBeSpecialSummoned(e,0,tp,true,false) end
	Duel.SetOperationInfo(0,CATEGORY_SPECIAL_SUMMON,e:GetHandler(),1,0,0)
end
function c100000767.spop(e,tp,eg,ep,ev,re,r,rp)
