...
function c500317511.spcon(e,tp,eg,ep,ev,re,r,rp)
	...
	return ec:IsControler(tp) and (ec:IsSetCard(0x85a) or ec:IsSetCard(0x85b) or ec:IsSetCard(0x3b)) and  ec:GetSummonType()==SUMMON_TYPE_SPECIAL and (ec:GetPreviousLocation(LOCATION_HAND)or ec:GetPreviousLocation(LOCATION_DECK) or ec:GetPreviousLocation(LOCATION_GRAVE))
end
...
function c500317511.splimit(e,c,sump,sumtype,sumpos,targetp,se)
	return c:IsLocation(LOCATION_EXTRA) and not c:IsSetCard(0x85a) and not c:IsSetCard(0x85b) and not c:IsSetCard(0x3b)
end
...
function c500317511.con(e,tp,eg,ep,ev,re,r,rp)
	...
	return (rc:IsSetCard(0x85a) or rc:IsSetCard(0x85b) or rc:IsSetCard(0x3b)) and bit.band(e:GetHandler():GetPreviousLocation(),LOCATION_ONFIELD)>0
end
function c500317511.ufilter(c,e,tp)
	return (c:IsSetCard(0x85a) or c:IsSetCard(0x85b) or c:IsSetCard(0x3b)) and c:IsCanBeSpecialSummoned(e,0,tp,false,false)
end
