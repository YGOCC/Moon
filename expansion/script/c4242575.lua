...
--	e2:SetTarget(c4242575.distg)
	...
	e5:SetCountLimit(1,42425751)
	e5:SetCost(c4242575.thcost)
	e5:SetTarget(c4242575.thtg)
	e5:SetOperation(c4242575.thop)
	c:RegisterEffect(e5)
...
function c4242575.thcost(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return e:GetHandler():IsAbleToRemoveAsCost() end
	Duel.Remove(e:GetHandler(),POS_FACEUP,REASON_COST)
end
function c4242575.thtg(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.IsExistingMatchingCard(c4242575.thfilter,tp,LOCATION_DECK,0,1,nil) end
	Duel.SetOperationInfo(0,CATEGORY_TOHAND,nil,1,tp,LOCATION_DECK)
end
function c4242575.thop(e,tp,eg,ep,ev,re,r,rp,chk)
		Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_ATOHAND)
	local g=Duel.SelectMatchingCard(tp,c4242575.thfilter,tp,LOCATION_DECK,0,1,1,nil)
...
function c4242575.discon(e,tp,eg,ep,ev,re,r,rp)
    ...
    local g=Duel.GetMatchingGroup(c4242575.thfilter,c:GetControler(),LOCATION_GRAVE,0,nil)
...
function c4242575.distg(e,tp,eg,ep,ev,re,r,rp,chk)
	local g=Duel.GetMatchingGroup(c4242575.thfilter,tp,LOCATION_EXTRA,0,nil)
...
function c4242575.disop(e,tp,eg,ep,ev,re,r,rp)
	local g=Duel.GetMatchingGroup(c4242575.thfilter,tp,LOCATION_GRAVE,0,nil)
	local rg=Group.CreateGroup()
	for i=1,2 do
		local tc=g:Select(tp,1,1,nil):GetFirst()
		if tc then
