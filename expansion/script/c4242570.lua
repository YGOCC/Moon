...
function c4242570.initial_effect(c)
	...
	e1:SetCountLimit(1,4242570+EFFECT_COUNT_CODE_OATH)
	e1:SetOperation(c4242570.activate)
	c:RegisterEffect(e1)
    --add counter
	...
	e6:SetTarget(c4242570.desreptg)
	e6:SetOperation(c4242570.desrepop)
	c:RegisterEffect(e6)
end
--Activation code
...
function c4242570.activate(e,tp,eg,ep,ev,re,r,rp)
	...
	local g=Duel.GetMatchingGroup(c4242570.thfilter,tp,LOCATION_DECK,0,nil)
	if g:GetCount()>0 and Duel.SelectYesNo(tp,aux.Stringid(4242570,0)) then
...
function c4242570.atkval(e,c)
	return e:GetHandler():GetCounter(0x99)*50
end
function c4242570.acop(e,tp,eg,ep,ev,re,r,rp)
...
function c4242570.ctfilter(c)
	return c:IsFaceup() and c:IsSetCard(0x666) or c:IsSetCard(0x666) and IsType(TYPE_SPELL)
end
function c4242570.ctop(e,tp,eg,ep,ev,re,r,rp)
	if eg:IsExists(c4242570.ctfilter,1,nil) then
		e:GetHandler():AddCounter(0x99,1)
	end
end
function c4242570.desreptg(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return not e:GetHandler():IsReason(REASON_RULE)
		and e:GetHandler():GetCounter(0x99)>=4 end
	return true
end
function c4242570.desrepop(e,tp,eg,ep,ev,re,r,rp)
