...
function c100000724.initial_effect(c)
	--atk up
	local e2=Effect.CreateEffect(c)
	e2:SetType(EFFECT_TYPE_FIELD+EFFECT_TYPE_CONTINUOUS)
	e2:SetRange(LOCATION_MZONE)
	e2:SetCode(EVENT_CHAIN_SOLVING)
	e2:SetCondition(c100000724.condition)
	e2:SetOperation(c100000724.operation)
	c:RegisterEffect(e2)
		--to hand
	local e1=Effect.CreateEffect(c)
	e1:SetDescription(aux.Stringid(100000724,0))
	...
	e1:SetTarget(c100000724.thtg)
	e1:SetOperation(c100000724.thop)
	c:RegisterEffect(e1)
		local e11=Effect.CreateEffect(c)
	e11:SetDescription(aux.Stringid(100000724,0))
	e11:SetCategory(CATEGORY_TOHAND+CATEGORY_SEARCH)
	e11:SetType(EFFECT_TYPE_SINGLE+EFFECT_TYPE_TRIGGER_O)
	e11:SetCode(EVENT_SUMMON_SUCCESS)
	e11:SetProperty(EFFECT_FLAG_DAMAGE_STEP+EFFECT_FLAG_DELAY)
	e11:SetTarget(c100000724.target)
	e11:SetOperation(c100000724.operation2)
	...
	e8:SetType(EFFECT_TYPE_FIELD+EFFECT_TYPE_TRIGGER_F)	
		e8:SetDescription(aux.Stringid(100000724,0))
	e8:SetCode(EVENT_PHASE+PHASE_END)
		e8:SetCountLimit(1)
	e8:SetRange(LOCATION_MZONE)
	e8:SetOperation(c100000724.sop)
	c:RegisterEffect(e8)
end
...
function c100000724.filter(c)
	return c:IsCode(100000719) and c:IsAbleToHand()
end
function c100000724.target(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.IsExistingMatchingCard(c100000724.filter,tp,LOCATION_DECK,0,1,nil) end
	Duel.SetOperationInfo(0,CATEGORY_TOHAND,nil,1,tp,LOCATION_DECK)
end
function c100000724.operation2(e,tp,eg,ep,ev,re,r,rp)
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_ATOHAND)
	local g=Duel.SelectMatchingCard(tp,c100000724.filter,tp,LOCATION_DECK,0,1,1,nil)
...
function c100000724.thtg(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	...
	if chk==0 then return Duel.IsExistingTarget(c100000724.tdfilter2,tp,LOCATION_ONFIELD,0,1,nil) end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_RTOHAND)
	local g=Duel.SelectTarget(tp,c100000724.tdfilter2,tp,LOCATION_ONFIELD,0,1,1,nil)
	Duel.SetOperationInfo(0,CATEGORY_TOHAND,g,1,0,0)
end
function c100000724.thop(e,tp,eg,ep,ev,re,r,rp)
