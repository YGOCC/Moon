...
function c240100383.initial_effect(c)
	...
	e0:SetTarget(c240100383.target1)
	e0:SetOperation(c240100383.operation1)
	c:RegisterEffect(e0)
	...
	e2:SetCondition(c240100383.condition2)
	e2:SetTarget(c240100383.target2)
	e2:SetOperation(c240100383.operation2)
	c:RegisterEffect(e2)
...
function c240100383.operation1(e,tp,eg,ep,ev,re,r,rp)
	...
	local g=tg:FilterSelect(tp,c240100383.filter,1,1,nil)
	local tc=g:GetFirst()
	if tc and Duel.SendtoHand(tc,nil,REASON_EFFECT)~=0 and tc:IsLocation(LOCATION_HAND) and Duel.GetLocationCount(tp,LOCATION_MZONE)>0 then
		...
		local sg=Duel.SelectMatchingCard(tp,c240100383.sfilter,tp,LOCATION_HAND,0,1,1,nil,e,tp,tc:GetCode())
		if sg:GetCount()>0 then
			Duel.SpecialSummon(sg,0,tp,tp,false,false,POS_FACEUP)
		end
	end
end
function c240100383.condition2(e,tp,eg,ep,ev,re,r,rp)
	if not re:IsHasProperty(EFFECT_FLAG_CARD_TARGET) then return end
	if not re:GetHandler():IsType(TYPE_TRAP) then return false end
	local tg=Duel.GetChainInfo(ev,CHAININFO_TARGET_CARDS)
	if not tg or not tg:IsExists(c240100383.cfilter,1,nil) then return false end
	return Duel.IsChainNegatable(ev)
end
function c240100383.target2(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return true end
...
function c240100383.operation2(e,tp,eg,ep,ev,re,r,rp)
	local c=e:GetHandler()
	if not c:IsRelateToEffect(e) then return end
	local tg=Duel.GetChainInfo(ev,CHAININFO_TARGET_CARDS)
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_RTOHAND)
